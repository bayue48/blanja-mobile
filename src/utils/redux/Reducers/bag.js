const bagReducer = (
  ...
  switch (action.type) {
      ...
      return {
        ...
        mybag: inCart
          ? prevstate.mybag.map((items) =>
              ...
              items.color === newItem.color &&
              items.size === newItem.size
                ? {...items, qty: items.qty + 1}
                : items,
            )
          : [...prevstate.mybag, {...action.data}],
        totalAmmount: inCart
          ? prevstate.totalAmmount + newItem.price
          : prevstate.totalAmmount + newItem.qty * newItem.price,
      };
    case 'INC_QTY':
      const IncreaseQty = prevstate.mybag.map((items) =>
        items.product_id === action.data.product_id &&
        items.color === action.data.color &&
        items.size === action.data.size
          ? {...items, qty: items.qty + 1}
          : items,
      );
      return {
    ...
    case 'ORDER_ITEMS':
      return {
        ...prevstate,
        mybag: prevstate.mybag.map((items) =>
          true
            ? {
                ...items,
                payment: action.data.payment,
                address: action.data.address,
                trxId: action.data.trxId,
              }
            : items,
        ),
