...
class HeaderTransparent extends Component {
  ...
  componentDidMount = () => {
    if (this.props.auth.level === 1) {
      axios
        .get(BASE_URL + '/notif/seller/' + this.props.auth.id)
        .then(({data}) => {
          this.setState({
            notify: data.data,
          });
    ...
    } else if (this.props.auth.level === 2) {
      axios
        .get(BASE_URL + '/notif/buyer/' + this.props.auth.id)
        .then(({data}) => {
          this.setState({
            notify: data.data,
          });
      ...
      listNotification = (
        <>
          <View style={{marginTop: 20}}>
            {notify &&
              notify.map(({title, message}, index) => (
                <CardNotif key={index} title={title} content={message} />
              ))}
          </View>
        </>
      );
    ...
    return (
      <Container>
        <Header transparent>
          ...
          <Body>
            <Title style={{color: 'black', marginLeft: 25, fontWeight: 'bold'}}>
              Notification
            </Title>
          </Body>
        </Header>
