...
class writeReview extends Component {
  ...
  getDataTrx = () => {
    axios
      .get(API_URL + '/transactions/trx/' + this.props.route.params.trxId)
      .then(({data}) => {
        console.log(data);
        this.setState({
  ...
  submitReview = () => {
    if (this.state.selectedProduct != 0) {
      ...
      axios
        .post(API_URL + '/user/addReview', reviewData)
        .then(({data}) => {
          alert(data.message);
      ...
      <Container>
        ...
        <Content padder style={{backgroundColor: '#f0f0f0'}}>
          <Text
            ...
            Beri ulasan untuk transaksi {this.props.route.params.trxId}
          </Text>
          ...
          <View
            ...
            <Picker
              ...
              />
              {listProduct &&
                listProduct.map(({product_id, product_name}) => {
                  return (
