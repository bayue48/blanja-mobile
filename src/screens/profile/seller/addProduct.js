...
class AddProduct extends React.Component {
  ...
  postProduct = () => {
    if (
      ...
      this.state.product_size !== 0 &&
      this.state.product_category !== 0 &&
      this.state.product_color !== 0 &&
      this.state.product_desc !== ''
    ) {
      const regexAlph = /^[A-Za-z]+$/;
      if (
        regexAlph.test(this.state.product_price) &&
        regexAlph.test(this.state.product_qty)
      ) {
        ToastAndroid.show(
          'Must number',
          ToastAndroid.SHORT,
    ...
    return (
      <>
        <Container>
          ...
          <Content
            style={{marginRight: 15, backgroundColor: 'white', marginTop: 20}}>
            <View>
              <Form>
