...
import {Container, Header, Body, Left, View, Text, Button} from 'native-base';
import {TextInput, ToastAndroid, Image, FlatList, Animated} from 'react-native';
...
const ChatRoom = ({navigation, route, setLoginfalse}) => {
  ...
  const seller = splitRoom[1];
  const buyer = splitRoom[0];
  ...
  const sendMessage = () => {
    if (message !== '') {
      ...
      console.log(Msg);
      axios
        .post(BASE_URL + '/chat/addMessage', Msg, config)
        .then(({data}) => {
          ToastAndroid.show(
            'Message Sent',
            ToastAndroid.SHORT,
  ...
  const getNewMessage = () => {
    axios
      .get(BASE_URL + '/chat/newMessage/' + room_id)
      .then(({data}) => {
        setChat(data.data);
      })
      .catch(({response}) => {
      ...
      <Container>
        ...
        <View style={{flexDirection: 'row'}}>
          ...
          <Button
            danger
            rounded
            style={{width: vh(10), height: vh(7), marginTop: 3}}
            onPress={sendMessage}>
            <Text>SEND</Text>
          </Button>
        </View>
...
const mapDispatchToProps = (dispatch) => {
  return {
    setLoginfalse: () => dispatch(setLoginfalse()),
  };
};
