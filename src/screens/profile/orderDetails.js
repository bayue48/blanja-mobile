...
class OrderDetails extends React.Component {
  ...
  getDataTransaksi = () => {
    axios
      .get(
        BASE_URL + '/transaksi/getOrderDetail/' + this.props.route.params.trxId,
      )
      .then(({data}) => {
        this.setState({
  ...
  changeStatus = (e) => {
    axios
      .patch(
        BASE_URL +
          `/transaksi/changeStatus/${e}/${this.state.orderDetails.TrxId}`,
      )
      .then(({data}) => {
        ToastAndroid.show(
          data.message,
  ...
  kirimPesanan = () => {
    ...
    if (
      this.state.trackNumb !== '' &&
      regexKurir.test(this.state.trackNumb) &&
      this.state.trackNumb.length >= 10
    ) {
      axios
        .patch(
          BASE_URL +
            `/transaksi/updateResi/${this.state.orderDetails.TrxId}/${this.state.trackNumb}`,
        )
        .then(({data}) => {
          ToastAndroid.show(
            data.message + ' input tracking number',
            ToastAndroid.SHORT,
            ToastAndroid.BOTTOM,
          );
          this.changeStatus(3);
      ...
      if (this.props.auth.level === 2) {
        btnAction = (
          <Button
            ...
            <Text>Confirm Order Complete</Text>
          </Button>
        );
      }
    } else if (status === 4) {
    ...
    return (
      <>
        <Container>
          ...
          <Content
            style={{backgroundColor: '#f0f0f0', marginHorizontal: vw(2)}}>
            <View style={{minHeight: vh(52)}}>
              <View
                style={{flexDirection: 'row', justifyContent: 'space-between'}}>
                <Text style={{fontWeight: 'bold', fontSize: 20}}>
                  Order No:<Text style={{color: 'gray'}}> {TrxId}</Text>
                </Text>
              ...
              {statusDelivery}
              <Text
                style={{fontWeight: 'bold', marginBottom: 15, marginTop: 10}}>
                {qty} Items
              </Text>

              {cardOrder &&
                cardOrder.map(
                  ({product_name, price, product_img, color, size, qty}) => {
                    return (
                      <>
                        <CardOrder
                          name={product_name}
              ...
              <Form>
                <Item>
                  ...
                  <Input
                    ...
                    value={trackNumb}
                    onChangeText={(text) => {
                      this.setState({trackNumb: text});
                    }}
                    placeholder="Input 10 Digit Tracking Number"
                  />
                </Item>
              </Form>
              <View
