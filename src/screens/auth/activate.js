...
class Register extends React.Component {
  ...
  Activate = () => {
    if (this.state.email === '' || this.state.otp === '') {
      ...
      axios
        .get(BASE_URL + `/auth/activate/${this.state.email}/${this.state.otp}`)
        .then(({data}) => {
          // alert(data.message)
  ...
  Resend = () => {
    if (this.state.email === '') {
      ...
      axios
        .post(BASE_URL + `/auth/resend`, resendData)
        .then(({data}) => {
          alert(data.message);
          // console.log(data)
        })
    ...
    return (
      <>
        <View style={{margin: 20}}>
          ...
          <Form>
            ...
            <TouchableOpacity
              ...
              <Text style={{fontWeight: 'bold'}}>
                Already activate account, Login here -{'>'}
              </Text>
            </TouchableOpacity>
            <Button full rounded danger onPress={this.Activate}>
          ...
          <TouchableOpacity style={{alignSelf: 'center'}} onPress={this.Resend}>
