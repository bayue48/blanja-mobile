...
class Review extends Component {
  ...
  componentDidMount = () => {
    ...
    axios
      .get(BASE_URL + '/user/getReview/' + this.props.idProduct)
      .then(({data}) => {
        this.setState({
          review: data.data,
          isEmpty: false,
  ...
  render() {
    ...
    if (isEmpty) {
      reviewItems = (
        <>
          <View style={{width: '100%', height: 50, marginVertical: 20}}>
            <Text style={{fontWeight: 'bold', fontSize: 20}}>No Review.</Text>
          </View>
        </>
      );
    } else {
      reviewItems = (
        <>
          {review &&
            review.map(({name, rating, review, created_at}) => {
              let starRating;
              if (rating === 1) {
                starRating = (
                  <>
                ...
                starRating = (
                  <>
                    <Image
                      source={require('../assets/icons/2stars.png')}
                      style={{marginBottom: 3}}
                    />
                  </>
                );
                ...
                starRating = (
                  <>
                    <Image
                      source={require('../assets/icons/3stars.png')}
                      style={{marginBottom: 3}}
                    />
                  </>
                );
                ...
                starRating = (
                  <>
                    <Image
                      source={require('../assets/icons/4stars.png')}
                      style={{marginBottom: 3}}
                    />
                  </>
                );
                ...
                starRating = (
                  <>
                    <Image
                      source={require('../assets/icons/5stars.png')}
                      style={{marginBottom: 3}}
                    />
                  </>
                );
      ...
      <>
        <View style={{marginRight: 15}}>
          ...
          <Grid>
            <Row>
              <Col size={2}>
                <Text style={{fontSize: 51, fontWeight: 'bold'}}>
                  {this.props.avgRating.toString().substr(0, 3)}
                </Text>
              </Col>
              <Col
                size={3}
                ...
                <Image
                  source={require('../assets/icons/5stars-removebg-preview.png')}
                  style={{marginBottom: 3}}
                />
                <Image
                  source={require('../assets/icons/4stars.png')}
              ...
              <Col
                size={1}
                style={{alignItems: 'flex-end', flexDirection: 'column'}}>
                <Text style={{fontSize: 10, color: '#9B9B9B', marginTop: 3}}>
                  {starFive}
                </Text>
                <Text style={{fontSize: 10, color: '#9B9B9B', marginTop: 3}}>
                  {starFour}
