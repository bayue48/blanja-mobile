...
class cardAddress extends React.Component {
  ...
  deleteAddress = () => {
    if (this.props.address.activeAddress === this.props.addressId) {
      Alert.alert('Failed!', 'Address is active as primary address', [
      ...
      axios
        .delete(BASE_URL + '/address/delete/' + this.props.addressId, config)
        .then(({data}) => {
          ToastAndroid.show(
        ...
        .catch(({response}) => {
          ...
          ToastAndroid.show('Failed!', ToastAndroid.SHORT, ToastAndroid.BOTTOM);
        });
    }
  };
  render() {
